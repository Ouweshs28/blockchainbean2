...
class MyContract extends Contract {
  ...
  async requestCertificate(ctx, farmerName, farmerId, address, cropName, ferlizers, farmArea, timestamp) {
    ...
    //TODO: 
    //do check to make sure the farmer exists in the blockchain
  ...
  async performTest(ctx, clinicalLaboratory, applicationID, testResults, labComments, timestamp) {
    ...
    if(testResults == 'PASS'){
      sampleCheck.certificateState = 'PASSED_QUALITY_CHECK_READY_FOR_SALE';
    }
    else{
  ...
  async approveCertificate(ctx, regulator, applicationId, certificateState, regulatorComments, timestamp) {
